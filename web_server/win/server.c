...
void setnonblocking_win(SOCKET sockfd){    u_long mode = 1; // 1 for non-blocking, 0 for blocking
...
void handle_form_submission(char *query_string) {
    ...
    if (input_start != NULL) {
        ...
        if (input_end == NULL) {
            // If there's no "&" character, consider the input value until the end of the string
...
int main() {
 ...
 time_t current_time=0;
    time(&current_time);
    printf("%s Accepted connection from %s:%d\n", ctime(&current_time), inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port) );
