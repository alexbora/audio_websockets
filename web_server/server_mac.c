...
#define debug(level, fmt, ...)                                                 \
  do {                                                                         \
    switch (level) {                                                           \
    case INFO:                                                                 \
      fprintf(stderr, "%s[INFO] %s:%d:%s(): " fmt, GREEN, __FILE__, __LINE__,  \
...
static inline void setnonblocking(int sockfd) {
  ...
  if (flags == -1)
    debug(WARN, "Error getting socket flags");
...
int main(int argc, char **argv) {
  ...
  kqueue_fd = kqueue();
  if (kqueue_fd == -1)
    debug(ERROR, "Kqueue creation failed");
