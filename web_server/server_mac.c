...
static inline void setnonblocking(int sockfd) {
  ...
  debug(INFO, "Socket set to non-blocking");
}

void handle_client(int client_socket, const char *file) {
#define BUFFER_SIZE 1024
  FILE *html_file = fopen(file, "r");
  if (html_file == NULL) {
...
const char *handle_args(int argc, char **argv) {
  return argc == 2 ? argv[1] : "index.html";
}
...
int main(int argc, char **argv) {
  ...
  if (sigaction(SIGINT, &sa, NULL) == -1)
    debug(WARN, "Error setting SIGINT handler");

  const char *file = handle_args(argc, argv);
  printf("file: %s\n", file);
      ...
      default: {
        if (event.ident == serverSocket) {
          ...
          while ((clientSocket =
            ...
            handle_client(clientSocket, file);
